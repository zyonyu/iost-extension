...
class Index extends Component<Props> {
  ...
  getTokenBalance = async () => {
    ...
    const url =
      account.network == 'MAINNET' ? 'https://api.iost.io' : account.network == 'LOCALNET' ? 'http://127.0.0.1:30001' : 'https://test.api.iost.io'
...
const IOSTBalance = ({
  ...
  const url = account
    ? `${
        account.network == 'MAINNET' ? 'https://www.iostscan.com' : account.network == 'LOCALNET' ? 'http://127.0.0.1:30001' : 'http://54.249.186.224'
