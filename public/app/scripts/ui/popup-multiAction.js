...
const i18n = {
  en: {
    ...
    Dapp_Tip3:
      '* If you add this contract to the white-list, it will allow you to sign directly when you initiate the same contract request to the same beneficiary, manual operation will be no longer applied.',
    ...
    return (
      <div className="multi-action">
        <div className="header">
          {hasSignMessage ? null : (
            <span onClick={this.toggleWhiteList} className={classnames('whitelist-btn', isAddWhitelist && 'active')}>
          ...
          <p className="memo">
            {tx.actions.length} {transLocal(lang, tx.actions.length > 1 ? 'XActionsNeedYouPermissionS' : 'XActionsNeedYouPermission')}
          </p>
        </div>
        <div className="content">
          <section className="list">
            <ol>
              {hasSignMessage
                ...
                : tx.actions.map(act => (
                    <li key={act.contract}>
                      {act.contract} <i className="arrow">-></i> {act.actionName}
                    </li>
                  ))}
            </ol>
          </section>
          <div className="detail">
            ...
            <div className="item">
              ...
              <div className="item-val">
                {tx.amount_limit.map(limit => {
                  let { value, token } = limit
                  if (token === 'iost') {
                    value = new Decimal(value).div(1000).toNumber()
                  }
                  return (
                    <span className="account-limit-item" key={token}>
                      {value} {token}
                    </span>
                  )
                })}
            ...
            {tx.actions.map((action, idx) => {
              ...
              let [token, contract, _account, amount, memo] = JSON.parse(action.data)
              if (token === 'iost') {
                amount = new Decimal(amount).div(1000).toNumber()
              }
              return (
                <React.Fragment>
                  <div className="group-tit">
                    {transLocal(lang, 'ActionX', idx + 1)} - {transLocal(lang, isTransfer ? 'Transfer' : 'Contract')}
                  </div>
                  {isTransfer ? (
                    <div key={idx} className={classnames(isTransfer && 'item-group')}>
                      ...
                      <div className="item">
                        ...
                        <p className="item-val" style={{ fontSize: 12 }}>
                          {memo}
                        </p>
                      </div>
                    </div>
        ...
        <div className="gas-setting">
          {!showGasForm && (
            <div className="transferAmount-box">
              ...
              <span className="iGAS-price" onClick={this.toggleGasForm}>
                {iGASPrice} iGas <i />
              </span>
            </div>
          )}
          {showGasForm && (
            <div className="gas-form">
              <div className="back-btn" onClick={this.toggleGasForm}>
                <i />
              </div>
              <div className="gas-form-item">
                <span>{transLocal(lang, 'Dapp_iGasPrice')}</span>
                <Input name="iGASPrice" value={iGASPrice} autoFocus onChange={this.handleFormChange} className="input" />
              </div>
              <div className="gas-form-item">
                <span>{transLocal(lang, 'Dapp_iGasAmount')}</span>
                <Input name="iGASLimit" value={iGASLimit} onChange={this.handleFormChange} className={classnames('input', 'iGASLimit')} />
              </div>
            </div>
          )}
        </div>
        <div className="footer">
          <button className={classnames('ft-btn', isAddWhitelist && 'whitelist')} onClick={this.cancelTx}>
            {transLocal(lang, 'Dapp_Cancel')}
          </button>
          <button className={classnames('ft-btn', 'primary', isAddWhitelist && 'whitelist')} onClick={this.confirmTx}>
            {transLocal(lang, 'Dapp_Confirm')}
          </button>
        </div>
      </div>
    )
